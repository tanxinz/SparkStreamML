...
object RelationBaseV2 extends IPropertiesTrait{

  // def calculationDistance(df1:DataFrame,df2:DataFrame) = {
  //   df1.join(df2,Seq(""),"outter")
  // }
  def CalcuteSameTime(df1:DataFrame,df2:DataFrame,time1start:String,time1end:String,time2start:String,time2end:String,
                      col1:String,col2:String,locationcol1:String,locationcol2:String) = {
                        //为避免在join阶段shuffle倾斜，date将会带上小时第一位，因为站点也只有一个
    val df1s = df1.withColumn("date",col(time1start).substr(0,13))
    val df2s = df2.withColumn("date",col(time2start).substr(0,13))
    val all = df1s.join(df2s,Seq("date"),"inner")
                  .select(df1s(col1),df2s(col2),df1s(locationcol1),df1s(time1start),df1s(time1end),df2s(locationcol2),df2s(time2start),df2s(time2end))
                  .filter(arr => TimeDistance.filter(arr.getString(3),arr.getString(4),arr.getString(6),arr.getString(7)))
    (all.filter(arr => arr.getString(2) == arr.getString(5)).groupBy(col1,col2).count.withColumn("count",col("count") * 1.0),
     all.filter(arr => arr.getString(2) != arr.getString(5)).groupBy(col1,col2).count.withColumn("count",col("count") * 1.0))
  }
